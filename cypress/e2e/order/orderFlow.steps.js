...
And("the order status should be {string}", (expectedStatus) => {
  // Get the stored order number
  const orderNumber = testContext.get('generated_order_number');
...
When("I select product {string}", (productId) => {
...
And("I set quantity to {int}", (quantity) => {
...
And("I select order type {string}", (orderTypeId) => {
...
And("I select shipping address {string}", (addressId) => {
